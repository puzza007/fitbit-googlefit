...
class Convertor:
    ...
    def EpochOfFitbitTimestamp(self, timestamp, tzincluded=False):
        ...
        if not tzincluded:
            logTime = dateutil.parser.parse(
                timestamp).replace(tzinfo=self.tzinfo)
        else:
    ...
    def ConvertFibitSleepPoint(self, date, data_point):
        ...
        if data_point['level'] == "light":
            ...
            sleepType = 72
        print("sleepType: {}".format(sleepType))
        return dict(
    ...
    def ConvertGFitSleepSession(self, sleep_points, logId):
        ...
        """
        minLogMillis = min([point['startTimeNanos']
                            for point in sleep_points]) / 10**6
        maxLogMillis = max([point['endTimeNanos']
                            for point in sleep_points]) / 10**6
    ...
    def ConvertFitbitActivityLog(self, activity):
        ...
        """
        startTimeMillis = self.EpochOfFitbitTimestamp(
            activity['startTime'], tzincluded=True)
        ...
        return dict(
            ...
            description='A Fitbit activity of type - {}'.format(
                activity['logType']),
    ...
    def GetDataSource(self, type='steps'):
        ...
        if type == 'steps':
            dataType = dict(name='com.google.step_count.delta', field=[
                            dict(name='steps', format='integer')])
        elif type == 'distance':
            dataType = dict(name='com.google.distance.delta', field=[
                            dict(name='distance', format='floatPoint')])
        elif type == 'weight':
            dataType = dict(name='com.google.weight', field=[
                            dict(name='weight', format='floatPoint')])
            model, device_type = 'aria', 'scale'  # weighing machine
        elif type == 'body_fat':
            dataType = dict(name='com.google.body.fat.percentage', field=[
                            dict(name='percentage', format='floatPoint')])
            model, device_type = 'aria', 'scale'  # weighing machine
        elif type == 'heart_rate':
            dataType = dict(name='com.google.heart_rate.bpm', field=[
                            dict(name='bpm', format='floatPoint')])
        elif type == 'calories':
            dataType = dict(name='com.google.calories.expended', field=[
                            dict(name='calories', format='floatPoint')])
        elif type in ('activity', 'sleep'):
            dataType = dict(name='com.google.activity.segment', field=[
                            dict(name='activity', format='integer')])
        else:
            raise ValueError("Unexpected data type given!")
    ...
    def GetDataSourceId(self, dataType):
        ...
        projectNumber = Storage(
            self.googleCredsFile).get().client_id.split('-')[0]
        return ':'.join((
            dataSource['type'],
