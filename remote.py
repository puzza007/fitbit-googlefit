...
class Remote:
    ...
    # Max number of data points that can be sent in a single update request
    ...
    def SyncFitbitIntradayToGoogleFit(self, dataType, date_stamp):
        ...
        googlePoints = [self.convertor.ConvertFibitPoint(
            date_stamp, point, dataType) for point in intraday_data]
    ...
    def SyncFitbitLogToGoogleFit(self, dataType, date_stamp):
        ...
        fitbitLogs = self.ReadFromFitbit(
            callMethod, base_date=date_stamp, end_date=date_stamp)[resp_id]

        # convert all fitbit data points to google fit data points
        googlePoints = [self.convertor.ConvertFibitPoint(
            date_stamp, point, dataType) for point in fitbitLogs]
    ...
    def SyncFitbitSleepToGoogleFit(self, date_stamp):
        ...
        fitbitSleeps = self.ReadFromFitbit(
            self.fitbitClient.get_sleep, date_obj)['sleep']

        # Iterate over each sleep log for that day
        sleep_count = 0
        for fit_sleep in fitbitSleeps:
            sleep_points = fit_sleep['levels']['data']

            # save first time stamp for comparison
            start_time = sleep_points[0]['dateTime']
            # convert to date, add 1 day, convert back to string
            next_date_stamp = (datetime.strptime(
                date_stamp, DATE_FORMAT) + timedelta(1)).strftime(DATE_FORMAT)

            # convert all fitbit data points to google fit data points
            googlePoints = [self.convertor.ConvertFibitPoint((date_stamp if start_time <= point['dateTime'] else
                                                              next_date_stamp), point, 'sleep') for point in sleep_points]
            sleep_count += len(googlePoints)

            # 1. Write a fit session about sleep
            google_session = self.convertor.ConvertGFitSleepSession(
                googlePoints, fit_sleep['logId'])
    ...
    def SyncFitbitActivitiesToGoogleFit(self, start_date='', callurl=None):
        ...
        activities_raw = self.ReadFromFitbit(
            self.fitbitClient.make_request, callurl)
        ...
        for activity in activities:
            ...
            activity_segment = dict(
                ...
                startTimeNanos=self.convertor.nano(
                    google_session['startTimeMillis']),
                endTimeNanos=self.convertor.nano(
                    google_session['endTimeMillis']),
        ...
        if activities_raw['pagination']['next'] != '':
            self.SyncFitbitActivitiesToGoogleFit(
